...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.src_word_emb0=nn.Embedding(n_src_vocab, d_word_vec, padding_idx=Constants.PAD) 
        self.src_word_emb1=nn.Embedding(n_src_vocab, d_word_vec, padding_idx=Constants.PAD) 
...
#             src_word_emb_output+=self.src_word_embs[i](src_seq[:,:,i])
        ...
        if not self.training and src_seq.shape[1] > hp.max_seq_len:
            enc_output = src_word_emb_output + get_sinusoid_encoding_table(src_seq.shape[1], hp.encoder_hidden)[:src_seq.shape[1], :].unsqueeze(0).expand(batch_size, -1, -1).to(src_seq.device)
        else:
            enc_output = src_word_emb_output + self.position_enc[:, :max_len, :].expand(batch_size, -1, -1)
